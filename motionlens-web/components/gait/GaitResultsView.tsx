...
// Display-only — the underlying DTO always contains both sides; the
// toggle just hides/shows series on the Plotly chart. Default
// "both" keeps existing behaviour byte-identical for any user who
...
function SideToggle({
  ...
  // Per-button colour: Both = orange (combined view), Left = red,
  // Right = yellow. Each button keeps its colour in every state —
  // inactive renders as a soft tint, active fills the full colour
  // with a halo ring. Operators can identify which side they're
  // looking at by the button colour itself.
  const opts: Array<{
    id: SideFilter;
    label: string;
    activeBg: string;
...
function AnkleTab({ data }: { data: GaitDataDTO }) {
  ...
  const passes = data.tabs_data.pass_segments;
  // Independent of the Overview toggle — each section owns its
  // own state. Default "both" preserves prior behaviour.
  const [sideFilter, setSideFilter] = useState<SideFilter>("both");
  const showLeft = sideFilter !== "right";
  const showRight = sideFilter !== "left";
