...
export function RehabCameraShell({
  ...
  // Draw-only spine smoothing state — the side/front verdict, two EMA
  // direction vectors and the front-bend offset + latch. Owned by the
  // shared overlay module; nothing here reaches onFrame or any
  // mechanic engine. One ref per camera, created once.
  ...
  const drawSkeleton = useCallback((landmarks: Norm[] | null) => {
    ...
    // Trunk (neck centreline + spine) — geometry lives in
    // lib/pose/spineOverlay.ts so other live shells can draw the
    // same thing. ctx is already translated into display space, which
    // is the coordinate system the helper documents.
