"use client";
// Live BlazePose capture for the Forward Lunge (B3) functional test.
// Lateral camera view, patient stepping forward into a lunge. Same
// structural pattern as the other orthopedic live cameras (dark
// canvas + skeleton overlay + selfie PiP + show/hide toggle); the
// window-level worst-rep capture helper is registered under
// __forwardLungeCapture so it doesn't collide with other tests on
// the page.
...
export function ForwardLungeLiveCamera({ onFrame, onError }: Props) {
  ...
  useEffect(() => {
    (window as unknown as { __forwardLungeCapture?: () => string | null }).__forwardLungeCapture = captureFrame;
    return () => {
      delete (window as unknown as { __forwardLungeCapture?: () => string | null }).__forwardLungeCapture;
    };
  }, [captureFrame]);
