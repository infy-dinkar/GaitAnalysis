...
export function ForwardLungeCapture() {
  ...
  // (the shell closes after a side finishes), so the countdown runs
    ...
    <div className="space-y-10">
                ...
                {sidesRemaining.length === 0 ? (
                  ...
                  <>
                    <p className="text-sm text-muted">
                      ...
                      finishes automatically after {TARGET_REP_COUNT} reps; once
                      both sides land the report saves to the patient record.
                    </p>
      ...
      {mode === "live" && liveFullscreen && (
        <LiveModeLayout
          ...
          subtitle={
            phase === "recording"
              ? `${liveSide === "left" ? "Left" : "Right"}-leg lunge · Rep ${Math.min(repsCaptured + 1, TARGET_REP_COUNT)} of ${TARGET_REP_COUNT}`
              : `${liveSide === "left" ? "Left" : "Right"}-leg lunge — camera on the ${liveSide ?? ""} side`
          }
          onExit={exitLive}
          camera={(
            <ForwardLungeLiveCamera
              onFrame={handleFrame}
              onError={setError}
              autoStart
              hideControls
            ...
            <>
              {flowPhase === "countdown" && countdown !== null && (
                <AutoFlowCountdownCard
                  ...
                  hint={`Patient standing tall, camera on the ${armedSide ?? "test"} side, full body in frame.`}
                />
              )}
                ...
                <div className="rounded-card border border-border bg-surface p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-medium text-foreground">
                      {liveSide === "left" ? "Left" : "Right"}-leg lunge
                    </p>
              ...
              <div className="rounded-card border border-border bg-surface p-3 text-xs text-muted">
                ...
                <ol className="mt-2 list-decimal space-y-1 pl-4">
                  <li>Standing tall, camera side-on to the {liveSide ?? "test"} (front) leg.</li>
                  <li>Step forward, front knee ≈ 90°, hold ~1 s, push back up — {TARGET_REP_COUNT} reps.</li>
                  <li>Auto-finishes on the {TARGET_REP_COUNT}th rep, then swap sides.</li>
                </ol>
              </div>
