"use client";
// D2 Tuck Jump (Myer's TJA) capture flow.
//
...
//      "Start camera" button. Nothing full-screen yet.
//   3. Operator clicks "Start camera" → fullscreen `LiveModeLayout`
//      mounts with `AssessmentCameraShell` on the left and a compact
//      calibration sidebar on the right. Everything from here onward
//      is immersive.
...
//
// UPLOAD mode: unchanged boxed form — file picker + height input +
// analyse button.
//
// Calibration math (`computeBodyPixelHeight`, `checkBodyInFrame`,
// `areReadingsStable`, `buildHeightCalibration`) is IMPORTED from
// `@/lib/calibration/heightCalibration` — the shared source of
// truth used by every calibrated test. `HeightCalibrationStep`
// (the boxed shared component) is NOT used here so the fullscreen
// shell can own the whole live surface without a nested camera.
...
export function TuckJumpCapture() {
  ...
  const [result, setResult] = useState<TuckJumpResult | null>(null);
  const [now, setNow] = useState<number>(0);

  // `cameraStarted` gates the pre-camera step (normal layout) vs the
  // fullscreen LiveModeLayout. `false` on first paint so the operator
  // sees just the height input + "Start camera" button.
  const [cameraStarted, setCameraStarted] = useState<boolean>(false);

  // Height input for LIVE calibration — pre-filled from patient record
  // when available. Lives here (not in LiveSection) so the value
  // survives the mount/unmount switch from pre-camera to fullscreen.
  ...
  // useState initializer above captures whatever patient.height_cm is
  // at mount time (often null while usePatientContext is still
  // hydrating). Only overwrite while the field is still empty so a
  // manual edit by the operator is preserved.
  useEffect(() => {
  ...
  async function uploadAndAnalyze(blob: Blob) {
    const file = new File([blob], "tuck_jump.webm", {
      type: blob.type,
    });
    setPhase("uploading");
    setError(null);
    try {
      const data = await analyzeTuckJumpUpload(file, calibration, null);
      setResult(data);
      setPhase("done");
    } catch (e) {
  ...
  // Reset LIVE state but keep the operator inside the fullscreen shell
  // and restart calibration. Used by the sidebar's Reset button.
  function reset() {
  ...
  // Fully exit LIVE mode — stops recorder, resets everything, and
  // drops back to the pre-camera step so the operator can re-enter
  // height / re-open the camera or switch to Upload.
  function exitLive() {
    reset();
  ...
  const [uploadResult, setUploadResult] = useState<TuckJumpResult | null>(null);
  ...
  async function analyzeUpload() {
    ...
    try {
      const r = await analyzeTuckJumpUpload(uploadFile, null, heightCm);
      setUploadResult(r);
      setUploadPhase("done");
    } catch (e) {
  ...
  if (liveDone || uploadDone) {
    ...
    const interpretation = buildTuckJumpInterpretation(r);
    return (
      <DoneView
...
interface LiveSectionProps {
  ...
  /** Reset LIVE state but stay inside the fullscreen shell (sidebar
   *  Reset button — restart calibration + recording). */
  onResetSession: () => void;
  /** Exit LIVE mode entirely — stops recorder, resets, unmounts the
   *  fullscreen shell (header Exit button). */
  onExitLive: () => void;
  error: string | null;
}
...
function LiveSection(props: LiveSectionProps) {
  ...
  // ── Pre-camera step — normal page layout, no fullscreen yet ────
  // Operator enters height, then clicks "Start camera". Calibration
  // and every phase after that render inside the fullscreen shell.
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-2 max-w-xl text-sm text-muted">
            The system will convert pixel heights to centimetres using
            the patient&apos;s standing height. Skipping the height step
            is allowed — Myer&apos;s scoring uses fraction-of-leg-length
            thresholds so classification stays valid in relative units.
          </p>
        </div>

        <div className="rounded-card border border-border bg-surface p-5">
          <label
            htmlFor="tuckjump-live-height"
  ...
  // ── Fullscreen: calibration acquisition + armed + recording +
  //    uploading — everything from Start-camera onward lives here.
  return (
    <FullscreenLiveShell
      phase={phase}
      calibration={calibration}
...
// Owns the inlined calibration state machine. Uses the shared math
// helpers from @/lib/calibration/heightCalibration so parity with
// HeightCalibrationStep is preserved.
...
function FullscreenLiveShell(props: FullscreenLiveShellProps) {
  ...
  // Per-frame callback — drives the stability gate during calibration
  // and forwards to the parent handleFrame in every phase.
      ...
      subtitle={
        calibration
          ...
          : isCalibrating
            ...
            : "Uncalibrated — pixel heights only (Myer's scoring stays valid)"
      }
      onExit={onExitLive}
      camera={(
          ...
          {isRecording && (
            <div className="absolute left-3 top-3 rounded-lg border border-white/15 bg-black/70 px-3 py-2 backdrop-blur">
              ...
              <p className="text-[10px] text-white/70">
                {targetProgress}% of Myer&apos;s {TARGET_SESSION_SEC}s target
              </p>
            </div>
          )}
          {isUploading && (
            ...
            <>
              <div>
                ...
                <p className="mt-1 text-xs text-muted">
                  Patient stands straight, full body facing the camera.
                  Skip to run in relative-units mode — Myer&apos;s scoring
            ...
            <>
              ...
              <div className="rounded-card border border-border bg-surface p-3 text-xs text-muted">
                ...
                <ol className="mt-2 list-decimal space-y-1 pl-4">
                  <li>Stand still ~1 s facing the camera.</li>
                  <li>
                    Perform continuous tuck jumps for
                    {" "}~{TARGET_SESSION_SEC}s — knees to chest at apex.
                  </li>
                  <li>Land on both feet, immediately re-jump.</li>
                </ol>
              </div>
              ...
              {isRecording && (
                <div className="rounded-card border border-rose-500/30 bg-rose-500/10 p-3 text-sm">
                  ...
                  <p className="mt-1 text-[11px] text-muted">
                    {targetProgress}% of Myer&apos;s target session.
                  </p>
                </div>
              )}
              {isUploading && (
    ...
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-semibold tracking-tight">
          Upload Tuck Jump video
        </h2>
        <p className="mt-2 text-sm text-muted">
          Single clip containing a continuous ~{TARGET_SESSION_SEC}s tuck-
          jump session (FRONTAL view). The backend derives scale
          calibration from the patient&apos;s height + the standing window
          of the clip.
        </p>
      </div>

      <div className="rounded-card border border-border bg-surface p-5">
        <label htmlFor="tuckjump-height" className="block text-sm font-medium">
          Patient height (cm)
        </label>
        <div className="mt-2 flex flex-wrap items-center gap-3">
          <input
        ...
        <label className="mt-3 flex items-center gap-2 text-xs text-muted">
          <input
            ...
            disabled={phase === "analyzing"}
          />
          Continue without calibration (Myer&apos;s scoring uses fraction-
          of-leg-length thresholds and stays valid without cm scaling)
        </label>
      </div>

      <FilePickerCard
        label="Tuck-jump session clip"
        file={file}
        onPick={onFile}
        disabled={phase === "analyzing"}
      />
