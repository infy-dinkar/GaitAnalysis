"use client";
// Live BlazePose capture for the Tandem Walk (E1) test. FRONTAL
// camera view, patient walking toward the lens. Same structural
// pattern as the other live cameras (dark canvas + skeleton overlay
// + selfie PiP + show/hide toggle); the window-level capture helper
// is registered under __tandemWalkCapture so it doesn't collide with
...
export function TandemWalkLiveCamera({ onFrame, onError }: Props) {
  ...
  // Composite screenshot helper for the saved report.
  ...
  useEffect(() => {
    (window as unknown as { __tandemWalkCapture?: () => string | null }).__tandemWalkCapture = captureFrame;
    return () => {
      delete (window as unknown as { __tandemWalkCapture?: () => string | null }).__tandemWalkCapture;
    };
  }, [captureFrame]);
