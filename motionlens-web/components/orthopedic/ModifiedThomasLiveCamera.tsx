"use client";
// Live BlazePose capture for the Modified Thomas Test. Lateral camera
// view, patient supine on the edge of a table with the hanging test
// leg in frame. Same structural pattern as the other orthopedic live
// cameras (dark canvas + skeleton overlay + selfie PiP + show/hide
// toggle); the window-level capture-frame helper is registered under
// __modifiedThomasCapture so it doesn't collide with other tests on
// the page.
...
interface Props {
  /** Per-frame callback. Keypoints are RAW (video pixel coords). */
  onFrame: (keypoints: Keypoint[], video: HTMLVideoElement) => void;
  onError?: (msg: string) => void;
}

export function ModifiedThomasLiveCamera({ onFrame, onError }: Props) {
  ...
  // Capture-moment screenshot — composite of mirrored video + skeleton.
  ...
  useEffect(() => {
    (window as unknown as { __modifiedThomasCapture?: () => string | null }).__modifiedThomasCapture = captureFrame;
    return () => {
      delete (window as unknown as { __modifiedThomasCapture?: () => string | null }).__modifiedThomasCapture;
    };
  }, [captureFrame]);
