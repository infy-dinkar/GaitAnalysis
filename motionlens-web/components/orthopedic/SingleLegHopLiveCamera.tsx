"use client";
// Live BlazePose capture for the D3 Single-Leg Hop test. Lateral
// view, patient hopping forward on the test leg. Mirrors
// FunctionalReachLiveCamera structurally — same dark canvas +
// skeleton overlay + PiP toggle. Exposes a window-level screenshot
// helper (__singleLegHopCapture) so the peak-frame screenshot is
...
export function SingleLegHopLiveCamera({ onFrame, onError }: Props) {
  ...
  useEffect(() => {
    (window as unknown as { __singleLegHopCapture?: () => string | null })
      .__singleLegHopCapture = captureFrame;
    return () => {
      delete (window as unknown as { __singleLegHopCapture?: () => string | null })
        .__singleLegHopCapture;
    };
  }, [captureFrame]);
