"use client";
// Live MoveNet capture for the 5x Sit-to-Stand test. Same PiP +
// dark-canvas convention as Trendelenburg / SingleLegSquat. Uses
// its own window-level capture helper so multiple test pages can
// be served concurrently without colliding.
...
export function SitToStandLiveCamera({ onFrame, onError }: Props) {
  ...
  useEffect(() => {
    (window as unknown as { __sitToStandCapture?: () => string | null }).__sitToStandCapture = captureFrame;
    return () => {
      delete (window as unknown as { __sitToStandCapture?: () => string | null }).__sitToStandCapture;
    };
  }, [captureFrame]);
