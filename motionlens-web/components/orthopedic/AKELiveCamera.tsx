"use client";
// Live BlazePose capture for the Active Knee Extension test. Lateral
// camera view, patient supine. Same structural pattern as the other
// orthopedic live cameras (dark canvas + skeleton overlay + selfie
// PiP + show/hide toggle) so the visual language is uniform; the
// window-level peak-frame helper is registered under __akeCapture so
...
export function AKELiveCamera({ onFrame, onError }: Props) {
  ...
  useEffect(() => {
    (window as unknown as { __akeCapture?: () => string | null }).__akeCapture = captureFrame;
    return () => {
      delete (window as unknown as { __akeCapture?: () => string | null }).__akeCapture;
    };
  }, [captureFrame]);
