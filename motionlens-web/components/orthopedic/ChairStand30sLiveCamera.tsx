"use client";
// Live MoveNet capture for the 30-Second Chair Stand test. Same
// PiP + dark-canvas convention as the other orthopedic tests.
// Owns its own window-level capture helper (`__chairStand30sCapture`)
// so multiple test pages can be served concurrently without
// colliding.
...
export function ChairStand30sLiveCamera({ onFrame, onError }: Props) {
  ...
  useEffect(() => {
    (window as unknown as { __chairStand30sCapture?: () => string | null }).__chairStand30sCapture = captureFrame;
    return () => {
      delete (window as unknown as { __chairStand30sCapture?: () => string | null }).__chairStand30sCapture;
    };
  }, [captureFrame]);
