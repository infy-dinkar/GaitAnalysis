"use client";
// Live BlazePose capture for the Pronator Drift (E2) test. FRONTAL
// camera, patient holding both arms extended at shoulder height. Same
// camera/skeleton-overlay structure as the other live cameras; the
// window-level capture-frame helper is registered under
// __pronatorDriftCapture so it doesn't collide with other tests.
...
export function PronatorDriftLiveCamera({ onFrame, onError }: Props) {
  ...
  useEffect(() => {
    (window as unknown as { __pronatorDriftCapture?: () => string | null }).__pronatorDriftCapture = captureFrame;
    return () => {
      delete (window as unknown as { __pronatorDriftCapture?: () => string | null }).__pronatorDriftCapture;
    };
  }, [captureFrame]);
