"use client";
// Live BlazePose capture for the D2 Tuck Jump test.
//
// Frontal view — patient faces the camera; both hips, knees and
// ankles must remain in frame throughout the ~10 s session so the
// engine can score Myer's valgus / symmetry / footprint items.
//
// Structural clone of CMJLiveCamera — same dark canvas + skeleton
// overlay + PiP toggle — with an isolated `window.__tuckJumpCapture`
// handle so the two tests don't collide when hosted on the same
// page or in dev tools.
...
export function TuckJumpLiveCamera({ onFrame, onError }: Props) {
  ...
  useEffect(() => {
    (window as unknown as { __tuckJumpCapture?: () => string | null })
      .__tuckJumpCapture = captureFrame;
    return () => {
      delete (window as unknown as { __tuckJumpCapture?: () => string | null })
        .__tuckJumpCapture;
    };
  }, [captureFrame]);
