"use client";
// Live MoveNet capture for the Single-Leg Stance test (Test C5).
// Same dark-canvas + PiP convention as Trendelenburg / B1 / C2 / C3.
...
export function SingleLegStanceLiveCamera({ onFrame, onError }: Props) {
  ...
  useEffect(() => {
    (window as unknown as { __singleLegStanceCapture?: () => string | null }).__singleLegStanceCapture = captureFrame;
    return () => {
      delete (window as unknown as { __singleLegStanceCapture?: () => string | null }).__singleLegStanceCapture;
    };
  }, [captureFrame]);
