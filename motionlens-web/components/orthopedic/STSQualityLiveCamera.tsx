"use client";
// Live BlazePose capture for the Sit-to-Stand Quality (B4) test.
// Lateral camera view, patient on a standard chair. Same structural
// pattern as the other orthopedic live cameras (dark canvas +
// skeleton overlay + selfie PiP + show/hide toggle); the window-level
// best-rep capture helper is registered under __stsQualityCapture so
// it doesn't collide with other tests on the page.
...
export function STSQualityLiveCamera({ onFrame, onError }: Props) {
  ...
  // Best-/worst-rep screenshot composite.
  ...
  useEffect(() => {
    (window as unknown as { __stsQualityCapture?: () => string | null }).__stsQualityCapture = captureFrame;
    return () => {
      delete (window as unknown as { __stsQualityCapture?: () => string | null }).__stsQualityCapture;
    };
  }, [captureFrame]);
