"use client";
// Live BlazePose capture for the C6 Functional Reach test. Lateral
// view, patient side-on. Mirrors SLRLiveCamera structurally — same
// dark canvas + skeleton overlay + PiP toggle — but exposes its own
// window-level screenshot helper (__functionalReachCapture) so the
// peak-frame screenshot is independent of other tests on the page.
...
export function FunctionalReachLiveCamera({ onFrame, onError }: Props) {
  ...
  useEffect(() => {
    (window as unknown as { __functionalReachCapture?: () => string | null })
      .__functionalReachCapture = captureFrame;
    return () => {
      delete (window as unknown as { __functionalReachCapture?: () => string | null })
