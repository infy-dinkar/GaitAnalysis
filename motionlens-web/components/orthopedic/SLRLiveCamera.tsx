"use client";
// Live BlazePose capture for the Straight Leg Raise test. Lateral
// camera view, patient supine. The component mirrors
// SingleLegSquatLiveCamera structurally (dark canvas + skeleton
// overlay + selfie PiP + show/hide toggle) so the visual language is
// uniform across orthopedic tests, but uses its own window-level
// capture helper (__slrCapture) so the peak-frame screenshot is
// independent of other tests on the page.
...
export function SLRLiveCamera({ onFrame, onError }: Props) {
  ...
  // Peak-frame screenshot — composite of mirrored video + skeleton.
  ...
  useEffect(() => {
    (window as unknown as { __slrCapture?: () => string | null }).__slrCapture = captureFrame;
    return () => {
      delete (window as unknown as { __slrCapture?: () => string | null }).__slrCapture;
    };
  }, [captureFrame]);
