...
function Inner() {
  ...
  // Auto-flow: Begin → 3-2-1 countdown → live → complete →
  // auto-save. Session-scoped refs reset at the live transition so
  // countdown framing noise never leaks into the payload.
  const {
    phase: sessionPhase,
    countdown,
    skipCountdown,
              ...
              sidebar={(
                <>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="inline-flex items-center gap-1.5 rounded-full bg-indigo-500/15 px-3 py-1 text-xs font-semibold text-indigo-200 ring-1 ring-indigo-400/40">Bilateral</span>
                  ...
                  {REHAB_EXERCISE_IMAGES["back-extension"] && (
                    <div className="overflow-hidden rounded-md border border-border bg-white">
                      ...
                      <img src={REHAB_EXERCISE_IMAGES["back-extension"]} alt="Back Extension reference" loading="lazy" className="block w-full object-contain" style={{ maxHeight: 140 }} />
                  ...
                  {sessionPhase === "countdown" && countdown !== null && (
                    <AutoFlowCountdownCard
                      ...
                      hint="Patient side-on to the camera, standing upright."
                    />
                  )}
                  {(sessionPhase === "live" || sessionPhase === "complete") && (
                    <div className="flex min-h-0 flex-1 flex-col">
                      <RepCountShell signal={trunkAngle} signalLabel="Trunk (°)" targetReps={TARGET_REPS} config={BACK_EXT_CONFIG} onSnapshot={handleSnapshot} compact />
                    </div>
                  )}
