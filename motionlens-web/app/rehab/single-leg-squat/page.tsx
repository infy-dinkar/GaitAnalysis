  ...
  const handleFrame = useCallback(
    (kp: Keypoint[], video: HTMLVideoElement) => {
      ...
      if (flexion !== null) {
        ...
        if (
          interiorAngle < SINGLE_LEG_SQUAT_CONFIG.topThreshold - 5
          && interiorAngle >= 40
  ...
  const buildRehabPayload = useCallback(() => {
    ...
    const interpretation = captured
      ? (reps > 0
          ...
          : `Deepest knee angle: ${deepestFlexionDeg!.toFixed(0)}°.`)
      : (reps > 0
          ? `${reps} single-leg squat${reps === 1 ? "" : "s"} counted. Knee depth not captured.`
    ...
    const skeletonPose = best
      ? {
          ...
          side,
          label: `Deepest single-leg squat — ${deepestFlexionDeg!.toFixed(0)}° knee angle`,
        }
      : fallback
        ? {
    ...
    return {
      ...
      metrics: {
        ...
        signal: {
          ...
          target_band: {
            min: 180 - SINGLE_LEG_SQUAT_CONFIG.topThreshold,
            max: 180 - SINGLE_LEG_SQUAT_CONFIG.depthThreshold,
          },
        },
    ...
    <>
      ...
      <main className="flex flex-col">
        <Section className="pt-32 md:pt-40">
          ...
          {side && (
            <LiveModeLayout
              title={`Single-Leg Squat · ${side === "left" ? "Left" : "Right"} leg`}
              ...
              camera={(
                <RehabCameraShell
                  ...
                  angleArc={{
                    ...
                    currentDeg: interior,
                    band: { min: SINGLE_LEG_SQUAT_CONFIG.depthThreshold, max: SINGLE_LEG_SQUAT_CONFIG.topThreshold },
                  }}
                >
                ...
                <>
                  ...
                  {REHAB_EXERCISE_IMAGES["single-leg-squat"] && (
                    <div className="overflow-hidden rounded-md border border-border bg-white">
                      ...
                      <img src={REHAB_EXERCISE_IMAGES["single-leg-squat"]} alt="Single-Leg Squat reference" loading="lazy" className="block w-full object-contain" style={{ maxHeight: 140 }} />
                  ...
                  {sessionPhase === "countdown" && countdown !== null && (
                    <AutoFlowCountdownCard
                      ...
                      hint="Patient side-on, standing tall on the working leg."
                    />
                  )}
                  {(sessionPhase === "live" || sessionPhase === "complete") && (
                    <div className="flex min-h-0 flex-1 flex-col">
                      <RepCountShell signal={interior} signalLabel="Knee angle (°)" signalDisplayName="knee_interior" targetReps={TARGET_REPS} config={SINGLE_LEG_SQUAT_CONFIG} onSnapshot={handleSnapshot} compact />
                    </div>
                  )}
