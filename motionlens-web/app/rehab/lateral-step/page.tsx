...
function Inner() {
    ...
    (kp: Keypoint[], video: HTMLVideoElement) => {
      ...
      if (flexion !== null) {
        ...
        if (interiorAngle < peakInteriorRef.current) {
          ...
          if (
            interiorAngle <= LATERAL_STEP_CONFIG.topThreshold
            && lastKpRef.current
          ) {
            bestPoseRef.current = {
  ...
  const buildRehabPayload = useCallback(() => {
    ...
    const skeletonPose = buildSkeletonPosePayload(
      ...
      side,
      `Deepest lateral-step load — ${peakInteriorRef.current.toFixed(0)}° knee interior`,
    );
    return {
      ...
      metrics: {
        ...
        signal: {
          ...
          target_band: {
            min: LATERAL_STEP_CONFIG.depthThreshold,
            max: LATERAL_STEP_CONFIG.topThreshold,
          },
        },
              ...
              sidebar={(
                <>
                  ...
                  <div className="flex min-h-0 flex-1 flex-col">
                    <RepCountShell signal={interior} signalLabel={`${side === "left" ? "L" : "R"} knee (°)`} targetReps={TARGET_REPS} config={LATERAL_STEP_CONFIG} onSnapshot={handleSnapshot} compact />
                  </div>
