...
export default function HipSetupPage() {
  return (
      ...
      <main className="flex flex-col">
        <Section className="pt-32 md:pt-40">
          <div className="max-w-2xl">
            <Badge>Hip assessment</Badge>
            <h1 className="mt-5 text-4xl font-semibold tracking-tight md:text-5xl">
              Configure the assessment<span className="text-accent">.</span>
            </h1>
            <p className="mt-5 text-lg text-muted">
              Hip flexion / extension and internal / external rotation. Set up the patient, pick a movement, capture live or upload a clip.
            </p>
          </div>
          ...
          <div className="mt-12 space-y-12">
            ...
            <section>
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                <Link
                  href={`/biomech/hip/live?movement=${movement ?? ""}&side=${side ?? ""}${patientQS}`}
                ...
                <Link
                  href={`/biomech/hip/upload?movement=${movement ?? ""}&side=${side ?? ""}${patientQS}`}
                  className={`rounded-card border p-6 transition ${
                    canProceed
                      ? "border-border bg-surface hover:border-accent"
                      : "pointer-events-none border-border/50 bg-surface/40 opacity-50"
