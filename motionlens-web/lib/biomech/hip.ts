...
export const HIP_MOVEMENTS: HipMovement[] = [
  ...
  // Mirrors the live-mode HIP_MOVEMENTS entry in hip-live.ts.
  {
    id: "rotation",
...
// ─── Compensation detection (Hip flexion / extension / rotation) ─
//
// All three exercises have their own tracker class:
//   • HipFlexionCompensationTracker  (standing lateral)
//   • HipExtensionCompensationTracker (standing lateral)
//   • HipRotationCompensationTracker  (seated frontal — full body in frame)
//
// Trunk angle convention: atan2(shoulder_mid.x − hip_mid.x,
//                                −(shoulder_mid.y − hip_mid.y))
//   → 0° at standing-upright (shoulder directly above hip)
//   → positive when shoulder shifts to image-right of hip
//   → negative when shoulder shifts to image-left of hip
//
// Posterior pelvic tilt (HIGH, hip flexion): trunk angle change
// magnitude during the leg-lift past 8°. The patient is leaning
// the torso back to gain leg height.
//
// Anterior pelvic tilt (HIGH, hip extension): same trunk-angle
// deviation signal but with the extension test — patient leans
// trunk forward to "throw" the leg back.
//
// Hip trunk lean (MEDIUM, flex+ext): shoulder.x displacement from
// baseline / baseline shoulder-width — captures any large trunk
// shift regardless of direction.
//
// Hip trunk rotation (HIGH, rotation): shoulder-width foreshortening,
// identical math to NeckRotationCompensationTracker's trunk_rotation.
//
// Knee tilt (MEDIUM, rotation): {side}_knee.x lateral deviation from
// baseline / baseline shoulder-width.
...
export function computeHipTrunkAngleDeg(
  keypoints: Keypoint[],
): number | null {
...
export class HipFlexionCompensationTracker extends HipBaseTracker {
  ...
  feed(keypoints: Keypoint[]): void {
    ...
    if (trunk !== null
        && this.trunkSamples.length < HIP_COMPENSATION_BASELINE_FRAME_COUNT) {
...
export class HipExtensionCompensationTracker extends HipBaseTracker {
  // Same baseline structure as HipFlexionCompensationTracker; only
  // the primary HIGH compensation label/type differs (anterior tilt).
  private trunkSamples: number[] = [];
  private shoulderXSamples: number[] = [];
  private widthSamples: number[] = [];
  private baselineTrunk: number | null = null;
...
export class HipRotationCompensationTracker extends HipBaseTracker {
  ...
  // Trunk rotation = shoulder-width foreshortening (identical math to
  // NeckRotationCompensationTracker's trunk_rotation signal).
  private widthSamples: number[] = [];
  private baselineWidth: number | null = null;
  private rotationPeakFrac = 0;
  private rotationFlagged = false;
  private currentRotationActive = false;
  // Knee tilt = side knee.x lateral deviation from baseline, scaled
  // by baseline shoulder-width.
