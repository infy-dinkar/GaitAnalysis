...
// ─── Compensation detection (Knee flexion + extension) ──────────
//
// Lateral view, stand-or-sit setup per instructions.ts. Two
// compensations, both baseline-relative whole-trial checks:
//
//   1. Thigh Movement (HIGH) — the thigh should stay roughly
//      stationary while only the lower leg bends. Patient lifting
//      or tilting the thigh inflates the "knee bend" reading by
//      adding hip motion. Signal: angle of the thigh vector
//      (hip → knee) vs first-10-frame baseline.
//
//   2. Knee Trunk Lean (MEDIUM) — patient leaning the torso to
//      assist the bend. Signal: trunk vector (hip-mid → shoulder-
//      mid) angle change from baseline.
//
// Lenient thresholds tuned to absorb BOTH standing and seated
// recordings (small natural body sway shouldn't false-fire).
...
/** Angle (deg) of the thigh vector (hip → knee) from image-y-down.
 *  Returns null when either landmark fails visibility. */
...
/** Angle (deg) of the trunk vector (hip-mid → shoulder-mid) from
 *  image-y-UP (standing-tall = ~0°). Returns null on visibility
 *  failure of either shoulder or either hip. */
...
export class KneeFlexExtCompensationTracker {
  ...
  // Peak markers (parity with shoulder/neck trackers; not used in
  // finish because both checks are baseline-relative whole-trial)
