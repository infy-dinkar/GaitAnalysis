...
/** Valid elbow flexion window (degrees) for the rotation test pose.
 *  The patient is supposed to hold the elbow tucked at the side, bent
 *  at ~90°. If the elbow is straighter than ~115° (arm hanging by the
 *  side) or sharper than ~65° (over-bent), the geometry behind the
 *  arcsin formula no longer holds: the forearm projection stops
 *  representing pure shoulder-axis rotation. We reject such frames so
 *  a patient who simply lowers their arm doesn't get a phantom 90°
 *  peak from the formula reading 'forearm vertical = sin⁻¹(1)'. */
 ...
 *
 *  Returns null when:
 *    • shoulder / elbow / wrist visibility drops, OR
 ...
 *  (or further), numerator and denominator grow together and the ratio
 *  stays anchored to the true rotation angle. The baseline's
 *  R_upperArmLength field is retained for diagnostics but no longer
 *  drives the live arcsin denominator. */
