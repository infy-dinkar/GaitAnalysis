...
3. The included `Dockerfile`:
   - Installs OpenCV / FFmpeg system libraries
   - Installs `requirements.txt` and `requirements_api.txt`
   - **Pre-downloads `pose_landmarker_lite.task`** so cold starts don't pay the network fetch
   - Runs `uvicorn api:app --host 0.0.0.0 --port $PORT` (HF injects `$PORT`, usually 7860)
4. Tighten CORS via the `MOTIONLENS_ALLOWED_ORIGINS` env var in Space settings.

### Vercel (frontend)

1. Import the repo, set the root directory to `motionlens-web/`.
2. Set `NEXT_PUBLIC_API_BASE_URL=https://<your-space>.hf.space` in Project Settings → Environment Variables.
3. Build command: `npm run build` (Vercel auto-detects Next.js).

### Streamlit Community Cloud (legacy)

1. Set the entrypoint to `app.py`.
2. `packages.txt` declares the Debian apt packages (libGL, libglib, ffmpeg, …).
3. `.python-version` pins Python 3.11.
